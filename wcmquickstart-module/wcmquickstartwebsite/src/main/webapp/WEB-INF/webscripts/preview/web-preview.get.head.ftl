<!-- Web Preview -->
<link rel="stylesheet" type="text/css" href="${url.context}/css/preview/web-preview.css" />

<script type="text/javascript" src="${url.context}/js/preview/swfobject.js"></script>
<script type="text/javascript" src="${url.context}/messages.js?locale=${locale}"></script>

<script type="text/javascript" src="${url.context}/js/preview/web-preview.js"></script>
