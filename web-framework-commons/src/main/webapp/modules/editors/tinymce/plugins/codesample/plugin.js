...
(function () {
    ...
    (function (Prism) {
    	...
    	var nestedRoundContent = '[^()<>]{0,256}(?:<[^<>]{0,256}>[^()<>]{0,256}){0,32}';
    	var nestedRound = '\\(' + nestedRoundContent + '(?:\\(' + nestedRoundContent + '(?:\\(' + nestedRoundContent + '\\)' + nestedRoundContent + '){0,32}\\)' + nestedRoundContent + '){0,32}\\)';
    	...
    	Prism.languages.insertBefore('csharp', 'class-name', {
    		...
    		'type-expression': {
    			...
    			pattern: re(/(\b(?:default|sizeof|typeof)\s*\(\s*(?!\s))<<0>>(?=\s*\))/.source, [typeExpression]),
    			lookbehind: true,
    			alias: 'class-name',
    			inside: typeInside
    		},
    	...
    	Prism.languages.insertBefore('csharp', 'string', {
    		'interpolation-string': [
    			{
    				pattern: re(/(^|[^\\])(?:\$@|@\$)"(?:[^\\{"]{0,256}(?:""|\\[\s\S]|\{\{)?(?:<<0>>[^\\{"]{0,256}(?:""|\\[\s\S]|\{\{)?)?[^\\{"]{0,256}){0,32}"/.source, [mInterpolation]),
    				lookbehind: true,
    				greedy: true,
    				inside: createInterpolationInside(mInterpolation, mInterpolationRound),
    			},
    			{
    				pattern: re(/(^|[^@\\])\$"(?:[^\\"{]{0,256}(?:\\.|\{\{)?(?:<<0>>[^\\"{]{0,256}(?:\\.|\{\{)?)?[^\\"{]{0,256}){0,32}"/.source, [sInterpolation]),
    				lookbehind: true,
    				greedy: true,
