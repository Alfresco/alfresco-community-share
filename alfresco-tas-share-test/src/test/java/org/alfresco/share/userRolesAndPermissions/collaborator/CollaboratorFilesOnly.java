...
public class CollaboratorFilesOnly extends ContextAwareWebTest {
  ...
  public void setupTest() {
    ...
    userService.create(adminUser, adminPassword, user2, password, user + "@tests.com", user2,
        user2);
  ...
  public void collaboratorCreateContent() {
    ...
    Assert.assertTrue(documentLibraryPage.isCreateContentMenuDisplayed(),
        "Create content menu is not displayed");
    ...
    LOG.info(
        "Step 3: Provide input for: Name= 'C8938test', Title= 'C8938test', Description= 'C8938test' and click the 'Create' button.");
    ...
    Assert.assertEquals(documentDetailsPage.getFileName(), "C8938test",
        "\"C8938test\" is not the file name for the file in preview");
    Assert.assertEquals(documentDetailsPage.getPropertyValue("Title:"), "C8938test",
        "\"C8938test\" is not the file title for the file in preview");
  ...
  public void collaboratorUploadContent() {
    ...
    assertTrue(documentLibraryPage.isContentNameDisplayed(testFile),
        String.format("File [%s] is displayed", testFile));
    assertTrue(documentLibraryPage.isContentNameDisplayed(testFile),
        String.format("File [%s] is displayed", testFile));
  ...
  public void collaboratorDownloadContent() {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8940,
        fileContent);
    ...
    Assert.assertTrue(isFileInDirectory(fileNameC8940, null),
        "The file was not found in the specified location");
  ...
  public void collaboratorViewInBrowser() {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8941,
        fileContent2);
  ...
  public void collaboratorEditInlineBySelf() {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8947,
        fileContent5);
    ...
    editInAlfrescoPage.sendDocumentDetailsFields(updatedDocName, updatedContent, updatedTitle,
        updatedDescription);
    ...
    assertEquals(documentDetailsPage.getPageTitle(), "Alfresco » Document Details",
        "Displayed page=");
    assertEquals(documentDetailsPage.getContentText(), updatedContent);
    assertTrue(documentDetailsPage.isPropertyValueDisplayed(updatedTitle),
        "Updated title is not displayed");
    assertTrue(documentDetailsPage.isPropertyValueDisplayed(updatedDescription),
        "Updated description is not displayed");
  ...
  public void collaboratorEditInlineByOthers() {
    ...
    assertTrue(documentDetailsPage.isPropertyValueDisplayed(updatedTitle1),
        "Updated title is not displayed");
    assertTrue(documentDetailsPage.isPropertyValueDisplayed(updatedDescription1),
        "Updated description is not displayed");
  ...
  public void collaboratorCancelEditingBySelf() {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user, password, siteName, DocumentType.MSWORD, fileNameC8957,
        fileContent9);

    LOG.info("Step 1: Mouse over the testFile and check available actions");
    documentLibraryPage.navigate(siteName);
    documentLibraryPage.renderedPage();
    Assert.assertTrue(documentLibraryPage.isContentNameDisplayed(fileNameC8957),
        String.format("Document %s is not present", fileNameC8957));
    ...
    Assert.assertEquals(googleDocsCommon.getConfirmationPopUpMessage(),
        "Editing in Google Docs™...", "Checking in Google Doc is not found.");
    browser.waitUntilElementDisappears(googleDocsCommon.confirmationPopup, 15L);
    ...
    Assert.assertTrue(googleDocsCommon.isLockedDocumentMessageDisplayed(),
        "Message about the file being locked is not displayed");
    Assert.assertTrue(googleDocsCommon.isGoogleDriveIconDisplayed(),
    ...
    googleDocsCommon.confirmFormatUpgrade();
    Assert.assertEquals(googleDocsCommon.getConfirmationPopUpMessage(),
        "Cancel Editing in Google Docs™", "Cancel Editing in Google Doc is not found.");
    browser.waitUntilElementDisappears(googleDocsCommon.confirmationPopup, 15L);
  ...
  public void collaboratorStartWorkflow() {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8962,
        fileContent10);
    ...
    LOG.info(
        "Step 4: On the new task workflow form provide the inputs and click on Start Workflow button.");
    ...
    Assert.assertEquals(create.getPageTitle(), "Alfresco » My Tasks",
        "My Tasks page is not opened");
  ...
  public void collaboratorUploadNewVersionSelfCreated() {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8942,
        fileContent3);
    ...
    documentLibraryPage.clickDocumentLibraryItemAction(fileNameC8942,
        language.translate("documentLibrary.contentAction.uploadNewVersion"), uploadContent);
    ...
    uploadContent.updateDocumentVersion(newVersionFilePath, "comments",
        UploadContent.Version.Major);
    documentLibraryPage.renderedPage();
   // browser.waitUntilElementVisible(documentLibraryPage.content(newVersionFile));
    browser.waitInSeconds(5);
    assertTrue(documentLibraryPage.isContentNameDisplayed(newVersionFile),
        String.format("File [%s] is displayed", newVersionFile));
    Assert.assertFalse(documentLibraryPage.isContentNameDisplayed(fileNameC8942),
        fileNameC8942 + " is displayed.");

    LOG.info("Step 4: Check the testFile content.");
    documentLibraryPage.clickOnFile(newVersionFile);
    assertEquals(documentDetailsPage.getContentText(), "updated by upload new version",
        String.format("Contents of %s are wrong.", newVersionFile));
    assertEquals(documentDetailsPage.getFileVersion(), "2.0",
        String.format("Version of %s is wrong.", newVersionFile));
    assertEquals(documentDetailsPage.getFileName(), newVersionFile,
        String.format("Name of %s is wrong.", newVersionFile));
  ...
  public void collaboratorUploadNewVersionOtherUserCreated() {
    ...
    contentService.createDocument(user2, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8943,
        fileContent4);
    setupAuthenticatedSession(user, password);
    ...
    LOG.info("Step 2: Click Upload New Version");
    documentLibraryPage.clickDocumentLibraryItemAction(fileNameC8943,
        language.translate("documentLibrary.contentAction.uploadNewVersion"), uploadContent);
    Assert.assertTrue(uploadContent.isUploadFilesToDialogDisplayed(), "Upload Files To Dialog is not displayed");

    LOG.info("Step 3: Select the updated version of testFile and confirm upload.");
    uploadContent.updateDocumentVersion(newVersionFilePath2, "comments",
        UploadContent.Version.Major);
    documentLibraryPage.renderedPage();
    //browser.waitUntilWebElementIsDisplayedWithRetry(documentLibraryPage.content(newVersionFile), 6);
    browser.waitInSeconds(5);
    assertTrue(documentLibraryPage.isContentNameDisplayed(newVersionFile2),
        String.format("File [%s] is displayed", newVersionFile2));
    Assert.assertFalse(documentLibraryPage.isContentNameDisplayed(fileNameC8943),
        fileNameC8943 + " is displayed.");

    LOG.info("Step 4: Check the testFile content.");
    documentLibraryPage.clickOnFile(newVersionFile2);
    assertEquals(documentDetailsPage.getContentText(), "updated by upload new version",
        String.format("Contents of %s are wrong.", newVersionFile2));
    assertEquals(documentDetailsPage.getFileVersion(), "2.0",
        String.format("Version of %s is wrong.", newVersionFile2));
    assertEquals(documentDetailsPage.getFileName(), newVersionFile2,
        String.format("Name of %s is wrong.", newVersionFile2));
  ...
  public void collaboratorCheckOutGoogleDocBySelf() throws Exception {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8953,
        fileContent7);

    LOG.info("Step 1: Mouse over the testFile and check available actions");
    documentLibraryPage.navigate(siteName);
    Assert.assertTrue(documentLibraryPage.isContentNameDisplayed(fileNameC8953),
        String.format("Document %s is not present", fileNameC8953));
    ...
    LOG.info("Step 3: Check the testFile status in Document Library.");
    browser.waitUntilWebElementIsDisplayedWithRetry(googleDocsCommon.lockedIcon);
    Assert.assertTrue(googleDocsCommon.isLockedIconDisplayed(), "Locked Icon is not displayed");
    Assert.assertTrue(googleDocsCommon.isLockedDocumentMessageDisplayed(),
        "Message about the file being locked is not displayed");
    Assert.assertTrue(googleDocsCommon.isGoogleDriveIconDisplayed(),
    ...
    browser.waitUntilElementDisappears(By.xpath("//span[contains(text(), 'Check In Google Doc™')]"), 9L);
    browser.waitUntilElementVisible(By.xpath("//div[contains(text(), 'Version Information')]"));
    ...
    googleDocsCommon.clickOkButton();
    googleDocsCommon.renderedPage();
    browser.waitUntilElementDisappears(By.xpath("//span[contains(text(), 'Checking In Google Doc™...')]"), 9L);
    ...
    Assert.assertTrue(googleDocsCommon.isDocumentNameUpdated(editedTitle),
        "Name of the document was not updated");
  ...
  public void collaboratorCheckOutGoogleDocByOthers() throws Exception {
    ...
    setupAuthenticatedSession(user, password);
    contentService.createDocument(user2, password, siteName, DocumentType.TEXT_PLAIN, fileNameC8954,
        fileContent8);

    LOG.info("Step 1: Mouse over the testFile and check available actions");
    documentLibraryPage.navigate(siteName);
    Assert.assertTrue(documentLibraryPage.isContentNameDisplayed(fileNameC8954),
        String.format("Document %s is not present", fileNameC8954));
    ...
    LOG.info("Step 3: Check the testFile status in Document Library.");
    browser.waitUntilElementVisible(googleDocsCommon.lockedIcon);
    Assert.assertTrue(googleDocsCommon.isLockedIconDisplayed(), "Locked Icon is not displayed");
    Assert.assertTrue(googleDocsCommon.isLockedDocumentMessageDisplayed(),
        "Message about the file being locked is not displayed");
    Assert.assertTrue(googleDocsCommon.isGoogleDriveIconDisplayed(),
    ...
    browser.waitUntilElementDisappears(By.xpath("//span[contains(text(), 'Check In Google Doc™')]"), 5L);
    browser.waitUntilElementVisible(By.xpath("//div[contains(text(), 'Version Information')]"));
    Assert.assertEquals(googleDocsCommon.isVersionInformationPopupDisplayed(), true);
    ...
    googleDocsCommon.clickOkButton();
    googleDocsCommon.renderedPage();
    browser.waitUntilElementDisappears(By.xpath("//span[contains(text(), 'Checking In Google Doc™...')]"), 5L);
    Assert.assertEquals(googleDocsCommon.isVersionInformationPopupDisplayed(), false);

    LOG.info("Step 8: Check the testFile status and confirm that file has been unlocked.");
    Assert.assertEquals(googleDocsCommon.checkLockedLAbelIsDisplayed(), false);
    Assert.assertEquals(googleDocsCommon.checkGoogleDriveIconIsDisplayed(), false);
    Assert.assertTrue(googleDocsCommon.isDocumentNameUpdated(editedTitle1),
        "Name of the document was not updated");
