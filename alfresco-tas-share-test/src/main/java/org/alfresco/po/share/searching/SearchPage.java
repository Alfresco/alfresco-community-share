...
public class SearchPage extends SharePage<SearchPage> implements AccessibleByMenuBar {
	...
	private By searchInDropdownOptionsSelector = By
			.cssSelector("div[id='FCTSRCH_SCOPE_SELECTION_MENU_GROUP'] td[class*='dijitMenuItemLabel']");
	...
	private By viewsDropdownOptionsSelector = By
			.cssSelector("#DOCLIB_CONFIG_MENU_VIEW_SELECT_GROUP .dijitMenuItemLabel");
	...
	public boolean isFilterOptionDisplayed(String filterId, String filterOption) {
		...
		return browser.findFirstElementWithValue(filterElement.findElements(By.cssSelector(".filterLabel")),
				filterOption) != null;
	}
	...
	public String getFilterOptionHits(String filterOption) {
		return browser.findFirstElementWithValue(allOptions, filterOption)
	...
	public boolean isSortDropdownComplete() {
		if (sortOptions.size() == 12)
			if (sortOptions.get(0).getText().equals("Relevance") && sortOptions.get(1).getText().equals("Name")
					&& sortOptions.get(2).getText().equals("Title")
					&& sortOptions.get(3).getText().equals("Description")
					&& sortOptions.get(4).getText().equals("Author") && sortOptions.get(5).getText().equals("Modifier")
					&& sortOptions.get(6).getText().equals("Modified date")
					&& sortOptions.get(7).getText().equals("Creator")
					&& sortOptions.get(8).getText().equals("Created date")
	...
	public void clickFilterOption(String option, String filterBy) {
		By selector = By
				.cssSelector("div.alfresco-documentlibrary-AlfDocumentFilters[id$='" + filterBy + "'] .filterLabel");
	...
	public int getFilterTypePosition(String filter) {
		return browser.findFirstElementWithValue(filterTypeList, filter)
				.findElements(By.xpath("ancestor::div[contains(@id, 'FCTSRCH_filter_')]/preceding-sibling::*")).size()
				+ 1;
	}
