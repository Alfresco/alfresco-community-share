...
if [[ "${COM_DEPENDENCY_VERSION}" =~ ^.+-SNAPSHOT$ ]] ; then
  pullAndBuildSameBranchOnUpstream "${COM_UPSTREAM_REPO}" "-Pags -Pall-tas-tests -Dlicense.failOnNotUptodateHeader=false $([ "${JOB_NAME,,}" = "test" ] && [ "${JOB_NAME:0:19}" = "community_shared_UI" ] && echo "-Pbuild-docker-images")"
else
  pullUpstreamTag "${COM_UPSTREAM_REPO}" "${COM_DEPENDENCY_VERSION}"

  if [ "${JOB_NAME,,}" = "test" ] && [ "${JOB_NAME:0:19}" = "community_shared_UI" ]; then
    buildUpstreamTag "${COM_UPSTREAM_REPO}" "${COM_DEPENDENCY_VERSION}" "-Pags -Pall-tas-tests -Dlicense.failOnNotUptodateHeader=false -Pbuild-docker-images"
  fi
fi
...
mvn -B -V install -DskipTests -Dmaven.javadoc.skip=true -Pags "-Dimage.tag=${TAG_NAME}" ${REPO_IMAGE} \
