...
function identifyUpstreamSourceBranch() {
  ...
  # use the current branch name (or in case of PRs, the target branch name)
...
function buildSameBranchOnUpstream() {
  ...
  mvn -B -V -q clean install -DskipTests -f packaging/tests/pom.xml
...
# In a shallow clone then we can't use "git tag" to list the tags.
function listRemoteTags() {
  git ls-remote --tags origin | grep -v "\^{}" | sed "s|^.*refs/tags/\(.*\)$|\1|g"
}

function nextARelease() {
  local ACS_VERSION="${1}"
